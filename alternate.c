...
#pragma omp parallel for if(nkernels == 128 && height >= 100 || nkernels >= 256 || height >= 200) private(m, x, y, kernel,index, kend, w, h, tempIndex, kVals, imgVals, cNumbs) shared(image, kernels, output)
    for (m = 0; m < nkernels; m++)
    {
        for (x = 0; x < kernel_order; x++)
        {
