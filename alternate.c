...
#pragma omp parallel for if(nkernels >= 128 && height >= 75 || height >= 150 || nkernels >= 256 && height >= 30 || nkernels >= 512) private(m, x, y, kernel,index, kend, w, h, tempIndex, kVals, imgVals, cNumbs) shared(image, kernels, output) schedule(static,1)
    for (m = 0; m < nkernels; m++)
    {
        for (x = 0; x < kernel_order; x++)
        {
