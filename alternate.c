...
#pragma omp parallel for if(nkernels == 128 && height >= 100 || nkernels >= 256 || height >= 150) private(m, x, y, kernel,index, kend, w, h, tempIndex, kVals, imgVals, cNumbs) shared(image, kernels) collapse(3)
    for (m = 0; m < nkernels; m++)
    {
        for (x = 0; x < kernel_order; x++)
        {
